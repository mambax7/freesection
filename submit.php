...
/**
* $Id: submit.php,v 1.25 2007/02/09 13:16:47 malanciault Exp $
* Module: FreeSection
...
Global $freesection_category_handler, $freesection_item_handler, $xoopsUser, $xoopsConfig, $xoopsModuleConfig, $xoopsModule;

// Get the total number of categories
$categoriesArray = $freesection_category_handler->getCategoriesForSubmit();

if (!$categoriesArray) {
	redirect_header("index.php", 1, _MD_FSECTION_NEED_CATEGORY_ITEM);
	exit();
}
...
switch ($op) {
	...
	$xoopsOption['template_main'] = 'freesection_submit.html';
	include_once(XOOPS_ROOT_PATH . "/header.php");
	include_once("footer.php");

	$name = ($xoopsUser) ? (ucwords($xoopsUser->getVar("uname"))) : 'Anonymous';

	$categoryObj = $freesection_category_handler->get($_POST['categoryid']);
	...
	if ($itemid) {
		$xoopsTpl->assign('categoryPath', _MD_FSECTION_EDIT_ARTICLE);
		$xoopsTpl->assign('lang_intro_title', _MD_FSECTION_EDIT_ARTICLE);
		//For RISQ
		$xoopsTpl->assign('lang_intro_text', '');
		//$xoopsTpl->assign('lang_intro_text', $myts->displayTarea(freesection_getConfig('submitintromsg')));
	} else {
		$xoopsTpl->assign('categoryPath', _MD_FSECTION_SUB_SNEWNAME);
		$xoopsTpl->assign('lang_intro_title', sprintf(_MD_FSECTION_SUB_SNEWNAME, ucwords($xoopsModule->name())));
		//For RISQ
		$xoopsTpl->assign('lang_intro_text', freesection_getConfig('submitintromsg'));
		//$xoopsTpl->assign('lang_intro_text', $myts->displayTarea(freesection_getConfig('submitintromsg')));

	}

	include_once FREESECTION_ROOT_PATH . 'include/submit.inc.php';
	...
	if ( $itemid || ($xoopsModuleConfig['autoapprove_submitted'] ==  1)) {
		...
		$itemObj->setVar('status', _FSECTION_STATUS_SUBMITTED);
	}

	// Storing the item object in the database
	if ( !$itemObj->store() ) {
		redirect_header("javascript:history.go(-1)", 2, _MD_FSECTION_SUBMIT_ERROR);
		exit();
	}
	...
	if (!$itemid) {
		if ( $xoopsModuleConfig['autoapprove_submitted'] ==  1) {
			...
			// Send notifications
			$itemObj->sendNotifications(array(_FSECTION_NOT_ITEM_PUBLISHED));

			$redirect_msg = _MD_FSECTION_ITEM_RECEIVED_AND_PUBLISHED;
		} else {
			...
			// Send notifications
			$itemObj->sendNotifications(array(_FSECTION_NOT_ITEM_SUBMITTED));

			$redirect_msg = _MD_FSECTION_ITEM_RECEIVED_NEED_APPROVAL;
		}
	} else {
		$redirect_msg = _MD_FSECTION_ITEMMODIFIED;
	}
	...
	if(isset($_GET['op']) && $_GET['op'] == 'clone'){
		$xoopsTpl->assign('categoryPath', _MD_FSECTION_CLONE);
		$xoopsTpl->assign('lang_intro_title', _MD_FSECTION_CLONE);
	}
	elseif ($itemid) {
		$xoopsTpl->assign('categoryPath', _MD_FSECTION_EDIT_ARTICLE);
		$xoopsTpl->assign('lang_intro_title', _MD_FSECTION_EDIT_ARTICLE);

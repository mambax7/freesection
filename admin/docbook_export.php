...
/**
* $Id: docbook_export.php,v 1.2 2005/07/24 17:27:25 malanciault Exp $
* Module: FreeSection
* Author: mariuss
* Licence: GNU
*/
...
if ($op == 'go')
{
    Header("Content-Disposition: attachment; filename=freesection.xml");
    Header("Connection: close");
    Header("Content-Type: text/xml; name=freesection.xml");
    ...
    $resultC = $xoopsDB->query ("select * from ".$xoopsDB->prefix("freesection_categories"));
    while ($arrC = $xoopsDB->fetchArray ($resultC))
    {
        ...
        $resultQ = $xoopsDB->query ("select * from ".$xoopsDB->prefix("freesection_item")." where categoryid=$c_categoryid");
        while ($arrQ = $xoopsDB->fetchArray ($resultQ))
        {
            ...
            $resultA = $xoopsDB->query ("select * from ".$xoopsDB->prefix("freesection_answers")." where answerid=$q_answerid");

<?php
/**
* $Id: news.php,v 1.3 2007/02/03 16:23:17 malanciault Exp $
* Module: SmartFAQ
...
$importFromModuleName = "News " . $_POST['from_module_version'];
...
if ($op == 'start')
{
	...
	freesection_collapsableBar('newsimport', 'newsimporticon',  sprintf(_AM_FSECTION_IMPORT_FROM, $importFromModuleName), _AM_FSECTION_IMPORT_INFO);
	...
	if ($totalCat == 0)
	{
		echo "<span style=\"color: #567; margin: 3px 0 12px 0; font-size: small; display: block; \">" . _AM_FSECTION_IMPORT_NO_CATEGORY . "</span>";
	}
	else
	{
...
if ($op == 'go')
{
	freesection_xoops_cp_header();
	freesection_adminMenu(-1, _AM_FSECTION_IMPORT);
	freesection_collapsableBar('newsimportgo', 'newsimportgoicon',  sprintf(_AM_FSECTION_IMPORT_FROM, $importFromModuleName), _AM_FSECTION_IMPORT_RESULT);
	...
	while ($arrCat = $xoopsDB->fetchArray ($resultCat))
	{
		...
		{
			echo sprintf(_AM_FSECTION_IMPORT_CATEGORY_ERROR, $arrCat['topic_title']) . "<br/>";
			continue;
		}
	...
	freesection_close_collapsable('newsimportgo', 'newsimportgoicon');

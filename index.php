...
/**
* $Id: index.php,v 1.55 2007/02/10 01:41:26 malanciault Exp $
* Module: FreeSection
...
global $freesection_category_handler, $freesection_item_handler, $xoopsUser, $xoopsModuleConfig;
...
if ($totalCategories  == 0 ) {
	redirect_header("../../index.php", 2, _MD_FSECTION_NO_TOP_PERMISSIONS);
	exit;
}
...
$categoriesObj = $freesection_category_handler->getCategories($xoopsModuleConfig['catperpage'], $catstart);

// if no categories are found, exit
$totalCategoriesOnPage = count($categoriesObj);
if ($totalCategoriesOnPage  == 0 ) {
	redirect_header("javascript:history.go(-1)", 2, _MD_FSECTION_NO_CAT_EXISTS);
	exit;
}


// Get subcats of the top categories
$subcats = $freesection_category_handler->getSubCats($categoriesObj);
...
$real_total_items = $freesection_item_handler->getCount();
...
if ($xoopsModuleConfig['displaylastitems']) {
	...
	$itemsObj = $freesection_item_handler->getAllPublished($xoopsModuleConfig['indexperpage'], $start, -1, $sort, $order);

	$totalItemsOnPage = count($itemsObj);
	$allcategories = $freesection_category_handler->getObjects(null, true);
	if ($itemsObj) {
		$userids = array();
		...
		for ( $i = 0; $i < $totalItemsOnPage; $i++ ) {
			...
			$item['categorylink'] = "<a href='" . XOOPS_URL . "/modules/freesection/category.php?categoryid=" . $itemsObj[$i]->categoryid() . "'>" . $allcategories[$itemsObj[$i]->categoryid()]->name() . "</a>";
...
/// sizeof($items);exit;
// Language constants
$xoopsTpl->assign(array('lang_on' => _MD_FSECTION_ON, 'lang_postedby' => _MD_FSECTION_POSTEDBY, 'lang_total' => isset($totalItemsOnPage) ? $totalItemsOnPage : '', 'lang_title' => _MD_FSECTION_TITLE, 'lang_datesub' => _MD_FSECTION_DATESUB, 'lang_hits' => _MD_FSECTION_HITS));
...
$xoopsTpl->assign('module_home', freesection_module_home(false));
$indexfooter = freesection_getConfig('indexfooter');
$indexfooter = $myts->displayTarea($indexfooter, 1);
$xoopsTpl->assign('indexfooter', $indexfooter);

$xoopsTpl->assign('lang_category_summary', _MD_FSECTION_INDEX_CATEGORIES_SUMMARY);
$xoopsTpl->assign('lang_category_summary_info', _MD_FSECTION_INDEX_CATEGORIES_SUMMARY_INFO);
$xoopsTpl->assign('lang_items_title', _MD_FSECTION_INDEX_ITEMS);
$xoopsTpl->assign('lang_items_info', _MD_FSECTION_INDEX_ITEMS_INFO);
 ...
 */
$freesection_metagen = new FreesectionMetagen($freesection_moduleName);
...
if($xoopsModuleConfig['show_rss_link'] == 1){
	$link=sprintf("<a href='%s' title='%s'><img src='%s' border=0 alt='%s'></a>",XOOPS_URL."/modules/freesection/backend.php", _MD_FSECTION_RSSFEED, XOOPS_URL."/modules/freesection/images/rss.gif",_MD_FSECTION_RSSFEED);

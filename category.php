...
/**
* $Id: category.php,v 1.51 2007/01/22 21:05:28 malanciault Exp $
* Module: FreeSection
...
if ($categoryObj->notLoaded()) {
    redirect_header("javascript:history.go(-1)", 1, _MD_FSECTION_NOCATEGORYSELECTED);
    exit();
}
...
$totalItems = $freesection_category_handler->publishedItemsCount();
...
if (!isset($totalItems[$categoryid]) || $totalItems[$categoryid] == 0) {
    //redirect_header("index.php", 1, _MD_FSECTION_MAINNOFAQS);
    //exit;
}
...
if ( empty( $xoopsOption['template_main'] ) ) {
	$xoopsOption['template_main'] = 'freesection_display' . '_' . $xoopsModuleConfig['displaytype'] . '.html';
}
...
$module = &$hModule->getByDirname('freesection');
...
$itemsObj = $freesection_item_handler->getAllPublished($xoopsModuleConfig['indexperpage'], $start, $categoryid, $sort, $order);
...
//$totalItems = $freesection_category_handler->publishedItemsCount($xoopsModuleConfig['displaylastitem']);

if ($xoopsModuleConfig['displaylastitem'] == 1) {
	// Get the last smartitem
	$last_itemObj = $freesection_item_handler->getLastPublishedByCat();
}
...
$subcatsObj = $freesection_category_handler->getCategories(0, 0, $categoryid);
...
if (count($itemsObj)>0) {
    ...
    for ( $i = 0; $i < $totalItemOnPage; $i++ ) {
		...
		$item['categorylink'] = "<a href='" . freesection_seo_genUrl('category', $itemsObj[$i]->categoryid(), $categoryObj->short_url()) . "'>" . $categoryObj->name() . "</a>";
...
$xoopsTpl->assign(array('lang_on' => _MD_FSECTION_ON, 'lang_postedby' => _MD_FSECTION_POSTEDBY, 'lang_total' => $totalItemOnPage, 'lang_title' => _MD_FSECTION_TITLE, 'lang_datesub' => _MD_FSECTION_DATESUB, 'lang_hits' => _MD_FSECTION_HITS));
$xoopsTpl->assign('sectionname', $freesection_moduleName);
$xoopsTpl->assign('whereInSection', $freesection_moduleName);
$xoopsTpl->assign('modulename', $xoopsModule->dirname());
$xoopsTpl->assign('lang_category_summary', sprintf(_MD_FSECTION_CATEGORY_SUMMARY,$categoryObj->name()));
$xoopsTpl->assign('lang_category_summary_info', sprintf(_MD_FSECTION_CATEGORY_SUMMARY_INFO,$categoryObj->name()));
$xoopsTpl->assign('lang_items_title', sprintf(_MD_FSECTION_ITEMS_TITLE,$categoryObj->name()));
$xoopsTpl->assign('lang_items_info', _MD_FSECTION_ITEMS_INFO);
...
$xoopsTpl->assign('lang_name_column',_MD_FSECTION_NAME);
$xoopsTpl->assign('lang_empty',_MD_FSECTION_EMPTY);
$xoopsTpl->assign('lang_view_more',_MD_FSECTION_VIEW_MORE);
$xoopsTpl->assign('selected_category',$categoryid);

//$xoopsTpl->assign('lang_items_list', _MD_FSECTION_ITEMS_LIST);
//$xoopsTpl->assign('lang_link_xplain', _MD_FSECTION_LINK_XPLAIN);
 ...
 */
$freesection_metagen = new FreesectionMetagen($categoryObj->getVar('name'), $categoryObj->getVar('meta_keywords','n'), $categoryObj->getVar('meta_description', 'n'), $categoryObj->getCategoryPathForMetaTitle());
...
if($xoopsModuleConfig['show_rss_link'] == 1){
	$link=sprintf("<a href='%s' title='%s'><img src='%s' border=0 alt='%s'></a>",XOOPS_URL."/modules/freesection/backend.php?categoryid=".$categoryid, _MD_FSECTION_RSSFEED, XOOPS_URL."/modules/freesection/images/rss.gif",_MD_FSECTION_RSSFEED);

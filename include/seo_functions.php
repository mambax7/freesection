...
// define FREESECTION_SEO_ENABLED in mainfile.php, possible values
...
function freesection_seo_title($title='', $withExt=true)
{
    /**
...
function freesection_seo_genUrl($op, $id, $short_url="")
{
    $smartModuleConfig = freesection_getModuleConfig();
    if (defined('FREESECTION_SEO_ENABLED'))
    {
    	if (! empty($short_url)) $short_url = $short_url . '.html';

        if (FREESECTION_SEO_ENABLED == 'rewrite')
        {
            // generate SEO url using htaccess
            return XOOPS_URL."/" . $smartModuleConfig['module_seo_modulename'] . ".${op}.${id}/${short_url}";
        }
        else if (FREESECTION_SEO_ENABLED == 'path-info')
        {
            // generate SEO url using path-info
            return XOOPS_URL."/modules/freesection/seo.php/${op}.${id}/${short_url}";
        }
        else
        {
    ...
    else
    {
       // generate classic url
        switch ($op) {
            case 'category':
               return XOOPS_URL."/modules/freesection/${op}.php?categoryid=${id}";
            case 'item':
            case 'print':
               return XOOPS_URL."/modules/freesection/${op}.php?itemid=${id}";
            default:

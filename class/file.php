...
/**
* $Id: file.php,v 1.21 2007/02/04 15:01:40 malanciault Exp $
* Module: FreeSection
...
// File status
define("_FSECTION_STATUS_FILE_NOTSET", -1);
define("_FSECTION_STATUS_FILE_ACTIVE", 1);
define("_FSECTION_STATUS_FILE_INACTIVE", 2);

class FreesectionFile extends XoopsObject
{
	/**
	* constructor
	*/
	function FreesectionFile($id = null)
	{
		...
		if (isset($id)) {
			...
			$file =& $freesection_file_handler->get($id);
	...
	function checkUpload($post_field, &$allowed_mimetypes, &$errors)
	{
	   ...
	   /* $maxfilesize = $config['uploadSize'];
        ...
        $uploader = new XoopsMediaUploader(freesection_getUploadDir(), $allowed_mimetypes, $maxfilesize, $maxfilewidth, $maxfileheight);
	...
	function storeUpload($post_field, $allowed_mimetypes = null, &$errors)
	{
        ...
        if(!is_dir(freesection_getUploadDir())){
            mkdir(freesection_getUploadDir(), 0757);
        }
	...
	function store(&$allowed_mimetypes, $force = true, $doupload =true)
	{
		...
		global $freesection_file_handler;
		return $freesection_file_handler->insert($this, $force);
	}
	...
	function getFileUrl()
	{
     	...
     	$freesection_module =& $hModule->getByDirname('freesection');
	 	$freesection_config = &$hModConfig->getConfigsByCat(0, $freesection_module->getVar('mid'));

		return freesection_getUploadDir(false) .  $this->filename();
	}
	...
	function getFilePath()
	{
     	...
     	$freesection_module =& $hModule->getByDirname('freesection');
	 	$freesection_config = &$hModConfig->getConfigsByCat(0, $freesection_module->getVar('mid'));
     	return 	freesection_getUploadDir() . $this->filename();
	}

	function getFileLink()
	{
		return "<a href='" . XOOPS_URL . "/modules/freesection/visit.php?fileid=" . $this->fileid() . "'>" . $this->name()	. "</a>";
	}

	function getItemLink() {
		return "<a href='" . XOOPS_URL . "/modules/freesection/item.php?itemid=" . $this->itemid() . "'>" . $this->name()	. "</a>";
	}
	...
	function displayFlash() {
		if (!defined('MYTEXTSANITIZER_EXTENDED_MEDIA')) {
			include_once(FREESECTION_ROOT_PATH . 'include/media.textsanitizer.php');
		}
...
class FreesectionFileHandler extends XoopsObjectHandler
{
	...
	* @return object FreesectionFile
	*/
	function &create($isNew = true)
	{
		$file = new FreesectionFile();
		if ($isNew) {
			$file->setNew();
		}
		return $file;
	...
	* @return mixed reference to the {@link FreesectionFile} object, FALSE if failed
	*/
	function &get($id)
	{
		if (intval($id) > 0) {
			$sql = 'SELECT * FROM '.$this->db->prefix('freesection_files').' WHERE fileid='.$id;
	...
	*
	* @param object $file reference to the {@link FreesectionFile} object
	* @param bool $force
	* @return bool FALSE if failed, TRUE if already present and unchanged or successful
	*/
	function insert(&$fileObj, $force = false)
	{
		if (strtolower(get_class($fileObj)) != 'freesectionfile') {
            return false;
        }
        if (!$fileObj->isDirty()) {
            return true;
		...
		if ($fileObj->isNew()) {
			$sql = sprintf("INSERT INTO %s (fileid, itemid, name, description, filename, mimetype, uid, datesub, `status`, notifypub, counter) VALUES (NULL, %u, %s, %s, %s, %s, %u, %u, %u, %u, %u)", $this->db->prefix('freesection_files'), $itemid,  $this->db->quoteString($name), $this->db->quoteString($description), $this->db->quoteString($filename), $this->db->quoteString($mimetype), $uid, time(), $status, $notifypub, $counter);
		} else {
			$sql = sprintf("UPDATE %s SET itemid = %u, name = %s, description = %s, filename = %s, mimetype = %s, uid = %u, datesub = %u, status = %u, notifypub = %u, counter = %u WHERE fileid = %u", $this->db->prefix('freesection_files'), $itemid, $this->db->quoteString($name), $this->db->quoteString($description), $this->db->quoteString($filename), $this->db->quoteString($mimetype), $uid, $datesub, $status, $notifypub, $counter, $fileid);
		}
	...
	*/
	...
	{
		if (strtolower(get_class($file)) != 'freesectionfile') {

			return false;
		}
		// Delete the actual file
		if (!freesection_deleteFile($file->getFilePath())){

			return false;
		}
		// Delete the record in the table
		$sql = sprintf("DELETE FROM %s WHERE fileid = %u", $this->db->prefix("freesection_files"), $file->getVar('fileid'));
	...
	*/
	...
	{
		if (strtolower(get_class($itemObj)) != 'freesectionitem') {
			return false;
		}
	...
	* @return array array of {@link FreesectionFile} objects
	*/
	function &getObjects($criteria = null, $id_as_key = false)
	{
		$ret = array();
		$limit = $start = 0;
		$sql = 'SELECT * FROM '.$this->db->prefix('freesection_files');
	...
	*/
	...
	{
		$sql = 'SELECT COUNT(*) FROM '.$this->db->prefix('freesection_files');
		if (isset($criteria) && is_subclass_of($criteria, 'criteriaelement')) {
			$sql .= ' '.$criteria->renderWhere();
		}
		$result = $this->db->query($sql);
	...
	*/
	...
	{
		$sql = 'DELETE FROM '.$this->db->prefix('freesection_files');
		if (isset($criteria) && is_subclass_of($criteria, 'criteriaelement')) {
			$sql .= ' '.$criteria->renderWhere();
		}
		if (!$result = $this->db->query($sql)) {
	...
	**/
	function updateAll($fieldname, $fieldvalue, $criteria = null)
	{
		...
		$sql = 'UPDATE '.$this->db->prefix('freesection_files').' SET '.$set_clause;
		if (isset($criteria) && is_subclass_of($criteria, 'criteriaelement')) {
			$sql .= ' '.$criteria->renderWhere();
		}
		//echo "<br />" . $sql . "<br />";

...
/**
* $Id: item.php,v 1.93 2007/02/09 16:02:56 malanciault Exp $
* Module: FreeSection
...
// ITEM status
define("_FSECTION_STATUS_NOTSET", -1);
define("_FSECTION_STATUS_ALL", 0);
define("_FSECTION_STATUS_SUBMITTED", 1);
define("_FSECTION_STATUS_PUBLISHED", 2);
define("_FSECTION_STATUS_OFFLINE", 3);
define("_FSECTION_STATUS_REJECTED", 4);

// Notification Events
define("_FSECTION_NOT_CATEGORY_CREATED", 1);
define("_FSECTION_NOT_ITEM_SUBMITTED", 2);
define("_FSECTION_NOT_ITEM_PUBLISHED", 3);
define("_FSECTION_NOT_ITEM_REJECTED", 4);

class FreesectionItem extends XoopsObject
{

    /**
     * @var FreesectionCategory
	 * @access private
     */
    var $_category = null;
	...
	* constructor
	*/
	function FreesectionItem($id = null)
	{
		...
		if (isset($id)) {
			...
			$item =& $freesection_item_handler->get($id);
	...
	{
    	$freesection_allCategoriesObj = freesection_getAllCategoriesObj();

		$this->_category = $freesection_allCategoriesObj[$this->getVar('categoryid')];

		global $freesection_permission_handler;
		if (!$freesection_permission_handler) {
			$freesection_permission_handler = xoops_getmodulehandler('permission', 'freesection')	;
		}
		$this->_groups_read = $freesection_permission_handler->getGrantedGroups('item_read', $this->itemid());
	...
	function checkPermission()
	{
		...
		$userIsAdmin = freesection_userIsAdmin();
		if ($userIsAdmin) {
			return true;
		}

		$freesectionPermHandler =& xoops_getmodulehandler('permission', 'freesection');

		$itemsGranted = $freesectionPermHandler->getGrantedItems('item_read');
    	...
    	if ($maxLength != 0) {
        	if (!XOOPS_USE_MULTIBYTES) {
                if (strlen($ret) >= $maxLength) {
                    $ret = freesection_substr($ret , 0, $maxLength);
                }
            }
        }
	...
	{
		$page = freesection_getUploadDir(true, 'content') . $file_name;
		if (file_exists($page)){
...
*/
	...
	function posterName($realName = -1)
	{
		...
		return freesection_getLinkedUnameFromId($this->uid(), $realName);
	}

	function updateCounter()
	{
		global $freesection_item_handler;
		return $freesection_item_handler->updateCounter($this->itemid());
	}
	...
	function store($force = true)
	{
		...
		if ( !$freesection_item_handler->insert($this, $force)) {
			return false;
		}

		if ($isNew && $this->status() == _FSECTION_STATUS_PUBLISHED) {
	...
	function getCategoryName()
	{
		...
		if (!isset ($this->_category)) {
			$this->_category = $freesection_category_handler->get($this->getVar('categoryid'));
		}
	...
	function getFiles()
	{
		...
		return $freesection_file_handler->getAllFiles($this->itemid(), _FSECTION_STATUS_FILE_ACTIVE);
	}
	...
	function getAdminLinks()
	{
		...
		$filePath = XOOPS_ROOT_PATH."/modules/freesection/language/".$xoopsConfig['language']."/main.php";
		if (file_exists($filePath)) {
			include_once(XOOPS_ROOT_PATH."/modules/freesection/language/".$xoopsConfig['language']."/main.php");
		} else {
			include_once(XOOPS_ROOT_PATH."/modules/freesection/language/english/main.php");
		}

		$adminLinks = '';

		// Find if the user is admin of the module
		$isAdmin = freesection_userIsAdmin();
		...
		if (is_object($xoopsUser) && (
			...
			if (!$isAdmin) {
				if ($xoopsUser->uid() == $this->uid()) {
					// Edit button
					$adminLinks .= "<a href='" . FREESECTION_URL . "submit.php?itemid=" . $this->itemid() . "'><img src='" . FREESECTION_URL . "images/links/edit.gif'" . " title='" . _MD_FSECTION_EDIT . "' alt='" . _MD_FSECTION_EDIT . "'/></a>";
					$adminLinks .= " ";
				}
				if (FREESECTION_LEVEL > 0) {
					if ($smartConfig['allowclone']) {
						// Dupplicate button
						$adminLinks .= "<a href='" . FREESECTION_URL . "submit.php?op=clone&itemid=" . $this->itemid() . "'><img src='" . FREESECTION_URL . "images/links/clone.gif'" . " title='" . _MD_FSECTION_CLONE . "' alt='" . _MD_FSECTION_CLONE . "'/></a>";
						$adminLinks .= " ";
					}
					...
					if ($smartConfig['allowupload']) {
						$uploadLink = "<a href='" . FREESECTION_URL . "addfile.php?itemid=" . $this->itemid() . "'><img src='" . FREESECTION_URL . "images/icon/file.gif' title='" . _MD_FSECTION_ADD_FILE . "' alt='" . _MD_FSECTION_ADD_FILE . "'/></a>";
					}
				}

			} else {
				// Edit button
				$adminLinks .= "<a href='" . FREESECTION_URL . "admin/item.php?op=mod&itemid=" . $this->itemid() . "'><img src='" . FREESECTION_URL . "images/links/edit.gif'" . " title='" . _MD_FSECTION_EDIT . "' alt='" . _MD_FSECTION_EDIT . "'/></a>";
				$adminLinks .= " ";

				if (FREESECTION_LEVEL > 0) {
					// Dupplicate button
					$adminLinks .= "<a href='" . FREESECTION_URL . "admin/item.php?op=clone&itemid=" . $this->itemid() . "'><img src='" . FREESECTION_URL . "images/links/clone.gif'" . " title='" . _MD_FSECTION_CLONE . "' alt='" . _MD_FSECTION_CLONE . "'/></a>";
					$adminLinks .= " ";
				}

				// Delete button
				$adminLinks .= "<a href='" . FREESECTION_URL . "admin/item.php?op=del&itemid=" . $this->itemid() . "'><img src='" . FREESECTION_URL . "images/links/delete.gif'" . " title='" . _MD_FSECTION_DELETE . "' alt='" . _MD_FSECTION_DELETE . "'/></a>";
				$adminLinks .= " ";
		...
		if (FREESECTION_LEVEL > 0) {
			// PDF button
			$adminLinks .= "<a href='" . FREESECTION_URL . "makepdf.php?itemid=" . $this->itemid() . "'><img src='" . FREESECTION_URL . "images/links/pdf.gif' title='" . _MD_FSECTION_PDF . "' alt='" . _MD_FSECTION_PDF . "'/></a>";
			$adminLinks .= " ";
		}

		// Print button
		$adminLinks .= '<a href="' . freesection_seo_genUrl("print", $this->itemid(), $this->short_url()) . '"><img src="' . FREESECTION_URL . 'images/links/print.gif" title="' . _MD_FSECTION_PRINT . '" alt="' . _MD_FSECTION_PRINT . '"/></a>';
		$adminLinks .= " ";

		// Email button
		$maillink = "mailto:?subject=" . sprintf(_MD_FSECTION_INTITEM, $xoopsConfig['sitename']) . "&amp;body=" . sprintf(_MD_FSECTION_INTITEMFOUND, $xoopsConfig['sitename']) . ": " . $this->getItemUrl();
		$adminLinks .= '<a href="' . $maillink . '"><img src="' . FREESECTION_URL . 'images/links/friend.gif" title="' . _MD_FSECTION_MAIL . '" alt="' . _MD_FSECTION_MAIL . '"/></a>';
		$adminLinks .= " ";
	...
	function sendNotifications($notifications=array())
	{
		...
		$tags = array();
		$tags['MODULE_NAME'] = $freesection_moduleName;
		...
		foreach ( $notifications as $notification ) {
			switch ($notification) {
				case _FSECTION_NOT_ITEM_PUBLISHED :
				...
				case _FSECTION_NOT_ITEM_SUBMITTED :
				...
				case _FSECTION_NOT_ITEM_REJECTED :
	...
	function getItemUrl()
	{
		return freesection_seo_genUrl('item', $this->itemid(), $this->short_url());
	}
	...
	{
	    $smartModuleConfig =& freesection_getModuleConfig();

		$posterName = freesection_getLinkedUnameFromId($this->uid(), $smartModuleConfig['userealname'], $users);
		$postdate = $this->datesub();
		return sprintf(_MD_FSECTION_POSTEDBY, $posterName, $postdate);
	}
	...
	function toArray($item_page_id = -1, $max_char_title=0)
	{
		...
		$item['categoryPath'] = $this->getCategoryPath(freesection_getConfig('linkedPath'));
		...
		if($this->showPartial_view()){
			$body = freesection_getConfig('partial_view_text');
		}
		else{
			$body = $this->body();
		}
		...
		if ($this->image() != 'blank.png') {
			$item['image_path'] = freesection_getImageDir('item', false) . $this->image();
		} else {
			$item['image_path'] = '';
		}
		...
		if($highlight && isset($_GET['keywords']))
		{
			...
			$h= new FreesectionKeyhighlighter ($keywords, true , 'freesection_highlighter');
	...
	function createMetaTags() {
		...
		$freesection_metagen = new FreesectionMetagen($this->title(), $this->meta_keywords(), $this->meta_description(), $this->_category->_categoryPath);
...
class FreesectionItemHandler extends XoopsObjectHandler
{

	function &create($isNew = true)
	{
		$item = new FreesectionItem();
		if ($isNew) {
	...
	* @return mixed reference to the {@link FreesectionItem} object, FALSE if failed
	*/
	function &get($id)
	{
		if (intval($id) > 0) {
			$sql = 'SELECT * FROM '.$this->db->prefix('freesection_items').' WHERE itemid='.$id;
	...
	*
	* @param object $item reference to the {@link FreesectionItem} object
	* @param bool $force
	...
	*/
	function insert(&$item, $force = false)
	{
        ...
        if (strtolower(get_class($item)) != 'freesectionitem') {
        	$item->setErrors('Invalid class provided.');
            return false;
        }

		if (!$item->meta_keywords() || !$item->meta_description() || !$item->short_url()) {
			$smartobject_metagen = new FreesectionMetagen($item->title(), $item->getVar('meta_keywords'), $item->getVar('summary'));
		...
		if ($item->isNew()) {
			...
			$this->db->prefix('freesection_items'),
		...
		if (freesection_tag_module_included()) {
			...
			$tag_handler->updateByItem($item_tag, $item->getVar('itemid'), 'freesection', 0);
		}

		// Saving permissions
		freesection_saveItemPermissions($item->getGroups_read(), $item->itemid());
	...
	*/
	function delete(&$item, $force = false)
	{
		...
		// Deleting the files
		global $freesection_file_handler;
		if (!$freesection_file_handler->deleteItemFiles($item)) {
			$item->setErrors('An error while deleting a file.');
		}

		$sql = sprintf("DELETE FROM %s WHERE itemid = %u", $this->db->prefix("freesection_items"), $item->itemid());
	...
	* @return array array of {@link FreesectionItem} objects
	*/
	function &getObjects($criteria = null, $id_key = 'none', $notNullFields='')
	{
		$ret = false;
		$limit = $start = 0;
		$sql = 'SELECT * FROM '.$this->db->prefix('freesection_items');
		...
		$freesection_items_read_group = $freesection_permission_handler->getGrantedGroupsForIds($itemsObj_array_keys, 'item_read');

		foreach ($theObjects as $theObject) {
			$theObject->_category = isset($freesection_allCategoriesObj[$theObject->getVar('categoryid')]) ? $freesection_allCategoriesObj[$theObject->getVar('categoryid')] : null;
			$theObject->_groups_read = isset($freesection_items_read_group[$theObject->itemid()]) ? $freesection_items_read_group[$theObject->itemid()] : array();
	...
	*/
	...
	{
		$sql = 'SELECT COUNT(*) FROM '.$this->db->prefix('freesection_items');
		if (isset($criteria) && is_subclass_of($criteria, 'criteriaelement')) {
			$whereClause = $criteria->renderWhere();
			if ($whereClause != 'WHERE ()') {
				$sql .= ' '.$criteria->renderWhere();
	...
	function getAllPublished($limit=0, $start=0, $categoryid=-1, $sort='datesub', $order='DESC', $notNullFields='', $asobject=true, $id_key='none')
	{
		...
		return $this->getItems($limit, $start, array(_FSECTION_STATUS_PUBLISHED), $categoryid, $sort, $order, $notNullFields, $asobject, $otherCriteria, $id_key);
	}
...
// Submited articles
	...
	{
		return $this->getItems($limit, $start, array(_FSECTION_STATUS_SUBMITTED), $categoryid, $sort, $order, $notNullFields, $asobject, null, $id_key);
	}
// Submited articles
// -------------------------------------------------------------------------------------------------------------------
...
// Offline articles
	...
	{
		return $this->getItems($limit, $start, array(_FSECTION_STATUS_OFFLINE), $categoryid, $sort, $order, $notNullFields, $asobject, null, $id_key);
	}
// Offline articles
// -------------------------------------------------------------------------------------------------------------------
...
// Rejected articles
	...
	{
		return $this->getItems($limit, $start, array(_FSECTION_STATUS_REJECTED), $categoryid, $sort, $order, $notNullFields, $asobject, null, $id_key);
	}
// Rejected articles
// -------------------------------------------------------------------------------------------------------------------
	function getItems($limit=0, $start=0, $status='', $categoryid=-1, $sort='datesub', $order='DESC', $notNullFields='', $asobject=true, $otherCriteria=null, $id_key='none')
	{
		...
		// Categories for which user has access
		if (!$userIsAdmin) {
			$categoriesGranted = $freesection_permission_handler->getGrantedItems('category_read');
			$grantedCategories = new Criteria('categoryid', "(".implode(',', $categoriesGranted).")", 'IN');
		}
		// Item for which user has access
		if (!$userIsAdmin) {
			$itemsGranted = $freesection_permission_handler->getGrantedItems('item_read');
			$grantedItem = new Criteria('itemid', "(".implode(',', $itemsGranted).")", 'IN');
		}
	...
	*/
	...
	{
		$sql = 'DELETE FROM '.$this->db->prefix('freesection_items');
		if (isset($criteria) && is_subclass_of($criteria, 'criteriaelement')) {
			$sql .= ' '.$criteria->renderWhere();
		}
		if (!$result = $this->db->query($sql)) {
	...
	**/
	function updateAll($fieldname, $fieldvalue, $criteria = null)
	{
		...
		$sql = 'UPDATE '.$this->db->prefix('freesection_items').' SET '.$set_clause;
		if (isset($criteria) && is_subclass_of($criteria, 'criteriaelement')) {
			$sql .= ' '.$criteria->renderWhere();
		}
	...
	{
		$sql = "UPDATE " . $this->db->prefix("freesection_items") . " SET counter=counter+1 WHERE itemid = " . $itemid;
	...
	$criteriaItemsStatus->add(new Criteria('item.status', _FSECTION_STATUS_PUBLISHED));
	...
	$sql = 'SELECT item.itemid, item.title, item.datesub, item.uid, item.categoryid FROM ('.$this->db->prefix('freesection_items') . ' as item) ';
	...
	$categoriesObj = $freesection_category_handler->getObjects($criteria, true);
	...
	function getLastPublishedByCat($status = array(_FSECTION_STATUS_PUBLISHED)) {

		$ret = array();
	    $itemclause = "";
   	    if (!freesection_userIsAdmin()) {
	        $freesectionPermHandler =& xoops_getmodulehandler('permission', 'freesection');
	        $items = $freesectionPermHandler->getGrantedItems('item_read');
	    ...
	    $sql2 = " LOCK TABLES ".$this->db->prefix('freesection_items')." READ;";
	    $sql3 = " INSERT INTO tmp SELECT categoryid, MAX(datesub) FROM ".$this->db->prefix('freesection_items')." WHERE status IN (". implode(',', $status).") $itemclause GROUP BY categoryid;";
	    $sql4 = " SELECT ".$this->db->prefix('freesection_items').".categoryid, itemid, title, short_url, uid, ".$this->db->prefix('freesection_items').".datesub FROM ".$this->db->prefix('freesection_items').", tmp
	                  WHERE ".$this->db->prefix('freesection_items').".categoryid=tmp.categoryid AND ".$this->db->prefix('freesection_items').".datesub=tmp.datesub;";
        /*
	    //Old implementation
	...
	function getCountsByCat($cat_id = 0, $status, $inSubCat=false) {
	    $ret = array();
	    $sql = 'SELECT c.parentid, i.categoryid, COUNT(*) AS count FROM '.$this->db->prefix('freesection_items') . ' AS i INNER JOIN '.$this->db->prefix('freesection_categories') . ' AS c ON i.categoryid=c.categoryid';
	    if (intval($cat_id) > 0) {

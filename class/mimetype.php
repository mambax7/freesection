<?php
//$Id: mimetype.php,v 1.3 2007/02/03 16:23:18 malanciault Exp $
//  ------------------------------------------------------------------------ //
...
require_once(FREESECTION_ROOT_PATH.'class/baseObjectHandler.php');

/**
 * freesectionMimetype class
 *
 * Information about an individual mimetype
 *
 * <code>
 * $hMime =& xoops_getModuleHandler('mimetype', 'freesection');
...
class freesectionMimetype extends XoopsObject {
    function freesectionMimetype($id = null)
    {
...
class freesectionMimetypeHandler extends freesectionBaseObjectHandler {
    /**
     * Name of child class
     * 
     * @var	string
     * @access	private
     */
	 var $classname = 'freesectionmimetype';
	 ...
	 * @access private
	 */
	var $_dbtable = 'freesection_mimetypes';
	 ...
	 */
	function freesectionMimetypeHandler(&$db) 
	{
		parent::init($db);
	}
	 ...
	 * @return	object	{@link freesectionMimetype}
	 * @access	public
	 */
	function &get($id)
	{
	 ...
	 * @return array array of {@link freesectionMimetype} objects
	 * @access	public	
	 */	
	...
	{
	    global $freesection_isAdmin, $xoopsUser, $xoopsModule;
	    
	    $ret = array();
	    if ($xoopsUser && !$freesection_isAdmin){
            // For user uploading
            $crit = new CriteriaCompo(new Criteria('mime_user', 1));   //$sql = sprintf("SELECT * FROM %s WHERE mime_user=1", $xoopsDB->prefix('freesection_mimetypes'));   
        } elseif ($xoopsUser && $freesection_isAdmin){
            // For admin uploading
            $crit = new CriteriaCompo(new Criteria('mime_admin', 1));  //$sql = sprintf("SELECT * FROM %s WHERE mime_admin=1", $xoopsDB->prefix('freesection_mimetypes'));
        } else {
            return $ret;
        }
        if($mime_ext){

...
/**
* $Id: category.php,v 1.42 2007/02/04 15:01:40 malanciault Exp $
* Module: FreeSection
...
class FreesectionCategory extends XoopsObject
{
    /**
     * @var array
	 * @access private
	...
	*/
	...
	{
		/*
		global $xoopsUser, $freesection_permission_handler;

		if (!$freesection_permission_handler) {
			$freesection_permission_handler = xoops_getmodulehandler('permission', 'freesection')	;
		}
		$this->_groups_read = $freesection_permission_handler->getGrantedGroups('category_read', $this->categoryid());
		$this->_groups_submit = $freesection_permission_handler->getGrantedGroups('item_submit', $this->categoryid());
		*/
	}
	...
	function checkPermission()
	{
		...
		$categoriesGranted = $freesectionPermHandler->getGrantedItems('category_read');
	...
	function getCategoryPath($withAllLink=true)
	{
		if (!$this->_categoryPath) {
			...
			if ($parentid != 0) {
				$parentObj =& $freesection_category_handler->get($parentid);
	...
	{
		return $this->itemsCount($inSubCat, $status=array(_FSECTION_STATUS_PUBLISHED));
	}

	function itemsCount($inSubCat=0, $status='')
	{
		Global $xoopsUser, $freesection_isAdmin;
		include_once XOOPS_ROOT_PATH.'/modules/freesection/include/functions.php';
		...
		$count = $freesection_item_handler->getCount($criteria);

		unset($criteria);

		if ($inSubCat) {
			include_once XOOPS_ROOT_PATH . "/class/xoopstree.php";
			$mytree = new XoopsTree($this->db->prefix("freesection_categories"), "categoryid", "parentid");
			...
			foreach ($subCats as $key => $value) {
				...
				$count = $count + $freesection_item_handler->getCount($criteria);
				unset($criteria);
			}
	...
	{
		return freesection_seo_genUrl('category', $this->categoryid(), $this->short_url());
	}
	...
	{
		global $freesection_category_handler, $freesection_permission_handler;

		$ret = $freesection_category_handler->insert($this, $force);
		...
		// Storing permissions
		//$freesection_permission_handler->saveItem_Permissions($this->getGroups_read(), $this->categoryid(), 'category_read');
		//$freesection_permission_handler->saveItem_Permissions($this->getGroups_submit(), $this->categoryid(), 'item_submit');

//		freesection_saveCategory_Permissions($this->getGroups_read(), $this->categoryid(), 'category_read');
//		freesection_saveCategory_Permissions($this->getGroups_submit(), $this->categoryid(), 'item_submit');
	...
	function toArray($category = array()) {
		...
		if ($this->image() != 'blank.png') {
			$category['image_path'] = freesection_getImageDir('category', false) . $this->image();

		} else {
			$category['image_path'] = '';
		}
		$category['lang_subcategories'] = sprintf(_MD_FSECTION_SUBCATEGORIES_INFO, $this->name());
		return $category;
	}

	function createMetaTags() {

		$freesection_metagen = new FreesectionMetagen($this->name(), $this->meta_keywords(), $this->meta_description());
...
class FreesectionCategoryHandler extends XoopsObjectHandler
{
	/**
	* create a new category
	*
	* @param bool $isNew flag the new objects as "new"?
	* @return object FreesectionCategory
	*/
	function &create($isNew = true)
	{
		$category = new FreesectionCategory();
		if ($isNew) {
			$category->setNew();
		}
		return $category;
	...
	* @return mixed reference to the {@link FreesectionCategory} object, FALSE if failed
	*/
	function &get($id)
	{
		if (intval($id) <= 0) {
	...
	*
	* @param object $category reference to the {@link FreesectionCategory} object
	* @param bool $force
	...
	*/
	function insert(&$category, $force = false)
	{
		...
		if (strtolower(get_class($category)) != 'freesectioncategory') {
			return false;
		}

		// Auto create meta tags if empty
		if (!$category->meta_keywords() || !$category->meta_description()) {
			$smartobject_metagen = new FreesectionMetagen($category->name(), $category->getVar('meta_keywords'), $category->getVar('description'));
		...
		if ($category->isNew()) {
			$sql = sprintf("INSERT INTO %s (
								...
								$this->db->prefix('freesection_categories'),
								$parentid,
	...
	*/
	function delete(&$category, $force = false)
	{
		...
		// Deleting the ITEMs
		global $freesection_item_handler;
		$items =& $freesection_item_handler->getItems(0, 0, -1, $category->categoryid());
		if ($items) {
			foreach ($items as $item) {
				$freesection_item_handler->delete($item);
			}
		}
		...
		$sql = sprintf("DELETE FROM %s WHERE categoryid = %u", $this->db->prefix("freesection_categories"), $category->getVar('categoryid'));
	...
	*/
	function &getObjects($criteria = null, $id_as_key = false)
	{
		$ret = array();
		$limit = $start = 0;
		$sql = 'SELECT * FROM '.$this->db->prefix('freesection_categories');
		...
		$freesection_category_group = $freesection_permission_handler->getGrantedGroupsForIds($itemsObj_array_keys);

		foreach ($theObjects as $theObject) {

			$theObject->_groups_read = isset($freesection_category_group['category_read'][$theObject->categoryid()]) ? $freesection_category_group['category_read'][$theObject->categoryid()] : array();
			$theObject->_groups_submit = isset($freesection_category_group['item_submit'][$theObject->categoryid()]) ? $freesection_category_group['item_submit'][$theObject->categoryid()] : array();
	...
	{
		global $freesection_isAdmin, $freesection_permission_handler, $theresult;

		$where_clause = '';

		if (!$freesection_isAdmin) {
		    $categoriesGranted = $freesection_permission_handler->getGrantedItems('item_submit');
		    $where_clause = " WHERE categoryid IN (" . implode(', ', $categoriesGranted) . ")";
		}

		$sql = "SELECT categoryid, parentid, name FROM " . $this->db->prefix('freesection_categories') . $where_clause . " ORDER BY name ASC";
	...
	*/
	...
	{
		$sql = 'SELECT COUNT(*) FROM '.$this->db->prefix('freesection_categories');
		if (isset($criteria) && is_subclass_of($criteria, 'criteriaelement')) {
			$sql .= ' '.$criteria->renderWhere();
		}
		$result = $this->db->query($sql);
	...
	*/
	...
	{
		$sql = 'DELETE FROM '.$this->db->prefix('freesection_categories');
		if (isset($criteria) && is_subclass_of($criteria, 'criteriaelement')) {
			$sql .= ' '.$criteria->renderWhere();
		}
	...
	**/
	function updateAll($fieldname, $fieldvalue, $criteria = null)
	{
		...
		$sql = 'UPDATE '.$this->db->prefix('freesection_categories').' SET '.$set_clause;
		if (isset($criteria) && is_subclass_of($criteria, 'criteriaelement')) {
			$sql .= ' '.$criteria->renderWhere();
		}
	...
	{
		return $this->itemsCount($cat_id, $status=array(_FSECTION_STATUS_PUBLISHED));
	}

	function itemsCount($cat_id = 0, $status='')
	{

		Global $xoopsUser, $freesection_item_handler;
		include_once XOOPS_ROOT_PATH.'/modules/freesection/include/functions.php';

		return $freesection_item_handler->getCountsByCat($cat_id, $status);
	}
